...
- [ ] **`docs/handoffs/Phase_9_Automatic_XP_Award_Sources.md`** — carried forward from Phase 9, still open. Automatic (event-driven) scene sharer/participant XP awards, and a forum weekly-post XP award via idempotent reconciliation (REQ-013).
- [ ] **`docs/handoffs/Phase_9_Character_Generation_UI.md`** — carried forward from Phase 9. **Codex paused on it and correctly identified three real architectural gaps rather than inventing contracts**, all now resolved and the handoff revised in place:
  1. There was genuinely no player-facing chargen B&B *removal* path — only the staff-only, 2-confirmation destructive `.delete`. Investigating this surfaced a deeper, previously-unnoticed bug: `SoulBnbApi.grant` unconditionally wrote Narrative History and fired an event on every call, which would have violated REQ-011's "no Narrative History before approval" rule the instant this handoff started actually calling it with real chargen selections. Fixed directly: `.grant` now defers history/event when `source == "chargen"` on an unapproved character; new `SoulBnbApi.drop_chargen_selection` (hard delete, chargen-only, pre-approval-only) added for removal; new `SoulBnbApi.finalize_chargen_grants` (idempotent, safe on re-approval) creates the deferred history at approval, wired into `custom-install/custom_approval.snippet.rb` alongside `lock_at_approval`. All specced (`plugin/spec/soul_bnb_api_spec.rb`).
  2. No canonical MUSH syntax existed for any chargen action. Resolved: new `+chargen` command family (`+chargen`, `/resonance`, `/skill`, `/bnb`, `/drop`) — confirmed `chargen` is an unclaimed root word in real core (which uses `app`/`bg`/`cg`/`hook`). Also corrected real scope creep in the original handoff: Aspect allocation was never actually part of FINAL REQ-011's canonical chargen flow (only Skill allocation is) and has been removed from scope entirely.
  3. The original handoff didn't identify the real web chargen mounting mechanism. Confirmed real: `chargen-custom-tabs.hbs`/`chargen-custom.hbs` (mirroring Inklings' own snippet pair) for the tab UI only — explicitly resolved that the generic `get_fields_for_chargen`/`save_fields_from_chargen`/`chargen-custom.js onUpdate()` data-batching mechanism (right for Inklings' simple text drafts) is the wrong fit for SOUL's structured, multi-record Skill/B&B data, and the mounted component should instead make direct real-time calls to new dedicated `soulChargen*` web operations, matching every other SOUL web component's existing pattern.
  Also added the small, real, previously-missing `SoulBnbApi.get_catalogue(chargen_available:)` filter kwarg the original handoff had left as a "check and maybe add" — confirmed it didn't exist and added it directly.
- [ ] **`docs/handoffs/Phase_10_Web_Command_Parity_Completion.md`** — written, handed to Codex: a staff SOUL administration surface (Framework/Resonance/Reload/Audit/XP-staff-actions/Culmination-staff-actions/B&B-staff-management, all currently web-API-only); roll modal additions (abort/force-abort/pending-list/history); B&B detail/search/scene-lookup UI (now backed by `soulBnbHere`); and a scene-scoped scene-GM sheet-viewing affordance (the backend `can_view?` already supports this via `scene_id`, but no component ever sends it). Design notes resolved directly: reuse the existing `can_manage_soul`/`soul_can_review_rolls` custom-field flags for all new gating rather than inventing new permission-flag mechanisms or probe calls.
