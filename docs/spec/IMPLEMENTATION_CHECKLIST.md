...
- [ ] **Real gap found 2026-07-24, prompted by a direct user question comparing to Grimoire's scene-page `cast` widget:** SOUL has no roll UI on the scene page at all — no component, no scene-related code anywhere in `web-portal/`. All backend web operations already exist (`soulRollStart`/`soulRollGm`/`soulRollSelect`/`soulRollReview`/`soulRollMark`, etc.); this is Ember-component-plus-mounting work only. Checked Grimoire's own `cast` component before recommending it as a model: it's scene-aware but **also never mounted onto an actual scene page anywhere in that repo** — a useful API-contract precedent, not a proven solution. Resolved that roll results should NOT auto-post to the scene transcript the way FS3/Grimoire's do — SOUL's own `+roll` command already never does this, consistent with REQ-031's privacy requirements. See `docs/handoffs/Phase_9_Scene_Page_Roll_Widget.md`.
