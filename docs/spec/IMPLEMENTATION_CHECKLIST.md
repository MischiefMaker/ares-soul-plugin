...
- [ ] **Real gap found 2026-07-24, prompted by a direct user question comparing to Grimoire's scene-page `cast` widget:** SOUL has no roll UI on the scene page at all — no component, no scene-related code anywhere in `web-portal/`. Checked Grimoire's own `cast` component before recommending it as a model: it's scene-aware but **also never mounted onto an actual scene page anywhere in that repo** — a useful API-contract precedent, not a proven solution. Resolved that roll results should NOT auto-post to the scene transcript the way FS3/Grimoire's do — SOUL's own `+roll` command already never does this, consistent with REQ-031's privacy requirements. **Revised same day after Codex's review correctly rejected the original "no backend changes needed" claim**: Codex identified three real, confirmed backend gaps (no player-facing suggested-B&B-candidate display anywhere — a genuine REQ-028 step 4 violation on both MUSH and web, not just this widget; no player-accessible difficulty-name lookup; `soulRollReview`'s pending-roll payloads omitting the rolling character, a real MUSH/web parity bug) plus a real UX issue with the originally-proposed permission-probe approach, and found the actual confirmed scene mounting point (`live-scene-custom-play.hbs`). All three backend gaps fixed directly (`SoulRollApi.get_player_candidate_view`/`.get_difficulty_options`, `pending_hash` now includes `character`, `+roll`'s own bare-status display also fixed since the candidate gap affected MUSH too) plus a `custom_scene_data.rb` snippet resolving the authorization UX cleanly. See the revised `docs/handoffs/Phase_9_Scene_Page_Roll_Widget.md`.
- [ ] **Real gap found during the 2026-07-24 checklist review, prompted by a direct user question about web parity:** the `web-portal/app/components/soul/*` Ember components (Sheet, XP, B&B, Culmination, History) and their backend web operations all exist and are correct, but **nothing mounts them into the actual character profile page** — no `custom-install/profile-custom-tabs.snippet.hbs`/`profile-custom.snippet.hbs`/`custom_char_fields.snippet.rb`, unlike Inklings, which ships all three for the identical purpose. A player cannot reach any SOUL web UI from their profile today. Separately, `xp.hbs`'s template has no skill picker/amount input/button to invoke the spend actions `xp.js` already implements correctly, and `sheet.hbs` never renders the `bnb` array `soulSheet` already returns. See `docs/handoffs/Phase_9_Profile_Tab_and_XP_Spend_UI.md`. **This retroactively means Phase 6's "both interfaces" claim below for Sheet/XP/B&B/Culmination/History was only true at the API layer, not the reachable-UI layer** — the web operations were real and correct, but nothing let a player actually navigate to them.
