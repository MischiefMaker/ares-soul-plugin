...
- [x] Plugin-wide `enabled` switch — `Soul.enabled?` now gates MUSH and web route registration, matching the existing profile-tab visibility flag. **Extended directly during merge review (2026-07-24):** Codex's change didn't gate `get_event_handler`, so a "disabled" SOUL would still silently process the weekly XP cron and scene-share events while every command/web route was locked out — added the same `return nil unless enabled?` guard there too, for full consistency.
