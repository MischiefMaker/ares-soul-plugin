...
**Fix:** converted every `custom-install/*.snippet.hbs` mounting invocation from curly-brace slash syntax to angle-bracket syntax (`<Soul::Sheet @character={{this.char.name}} />`, `<Soul::Chargen />`, `<Soul::Roll @scene={{this.scene}} @custom={{this.custom}} />`, etc. — `profile-custom.snippet.hbs`, `chargen-custom.snippet.hbs`, `live-scene-custom-play.snippet.hbs`). No component files moved or renamed — angle-bracket invocation resolves a nested component at the exact same `app/components/soul/<name>.js` / `app/templates/components/soul/<name>.hbs` path, just through a different, less resolver-version-sensitive code path than bare curly-brace slash invocation, and classic `Component.extend`-based components (which is what every SOUL component is) receive `@argName` the same way curly `argName=` already did (a plain `this.argName` property, not Octane's `this.args.argName`) - functionally identical, this is purely an invocation-syntax change. **Not yet confirmed fixed against the live game** - the user needs to replace their already-pasted old snippet content with the new angle-bracket version and rebuild before this can be marked verified; each snippet file now documents the change and instructs replacing old curly-brace content if already installed.
