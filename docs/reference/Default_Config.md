...
Complete default configuration file for SOUL, matching the canonical defaults in `docs/reference/Configuration.md` (derived from `SOUL_LLM_Implementation_Specification_FINAL.md` and `Implementation_Specification_Addendum.md`). This mirrors the file shipped at `game/config/soul.yml` in the plugin repository — copy it to your game's `game/config/` directory as a starting point (or let `plugin/install` merge it automatically).
...
# equivalent web portal action) - no plugin restart needed.
...
soul:
  ...
  framework:
    ...
    # Aspects have no post-chargen advancement path (no XP-spend equivalent
    # exists for them, unlike Skills) - this range applies for the
    # character's whole lifetime, not just chargen. Correctable afterward
    # only via +soul/framework/aspect (staff).
    aspect_min_rating: 0
    aspect_max_rating: 5
  ...
  resonance:
    ...
    # Chargen Aspect point-buy pool (Body/Mind/Spirit). Same shape as the
    # Skill point allowance above: a base at R0, plus a per-level rate that
    # can differ for positive vs. negative Resonance.
    r0_aspect_points: 5
    positive_aspect_points_per_level: 1
